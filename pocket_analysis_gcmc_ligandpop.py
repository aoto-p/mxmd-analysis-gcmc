...
##modified for population analysis, ensure alignments is on protein / single chain if multimeric
##calcualtes populations per probe, energies, and activities for use in qsar/pharmacophore
##requires manual rotation/translation (use rot.py script and alignment matrix) - to align dfe/fep frame to mxmd/3dpharmacophore and back
'''
#global:/
...
#$SCHRODINGER/run conformer_cluster.py -a 'res.ptype IMD' -in_place -HOST sge_cpu -n 0 -l Ward -rep -o clustered -OVERWRITE all_ligands.maegz -WAIT

def conformer_cluster_cosolv(probe, ptype):
    print('preparing cosolvent clustering')
    asl = f"'res.ptype {ptype}'"
    #split just probes for faster clustering and prep probes for 
    cmd = ['$SCHRODINGER/run', 'split_structure.py', '-k', '-m', 'ligand', '-ligand_asl', asl,  '-many_files', (probe+'_poses.maegz'), 'split.maegz']
    p = sp.Popen(' '.join(cmd), stdout = sp.PIPE, stderr=sp.PIPE, shell = True)
    out,err = p.communicate()

    cmd = ['$SCHRODINGER/run', 'structcat.py', 'split_ligand*.maegz', '-o' ,'all_ligands.maegz']
    p = sp.Popen(' '.join(cmd), stdout = sp.PIPE, stderr=sp.PIPE, shell = True)
    out,err = p.communicate()
    #cleanup
    cleanupfiles= glob.glob('split_*.maegz')
    for cleanupfile in cleanupfiles:
        if os.path.isfile(cleanupfile):
            os.remove(cleanupfile)

        #if os.path.isfile(file_path):
    print('submitted conformer_cluster.py for probe...')
    cmd = ['$SCHRODINGER/run','conformer_cluster.py','-a',asl,'-HOST','sge_cpu','-in_place','-n','0','-l','"Ward"','-rep','-o','clustered','-OVERWRITE','all_ligands.maegz','-WAIT']
    p = sp.Popen(' '.join(cmd), stdout = sp.PIPE, stderr=sp.PIPE, shell = True)
    out,err = p.communicate()
    #print(out,err)
...
#        return(int(frame_centroid))
    ...
    #unique_labels = cluster_order['ClusterNumber'].drop_duplicates().to_list() #get the cluster labels for the 3 highest pop
...
def extract_centroid(centroid_frame,cluster_number,cluster_pop,probe,pocket_name,pocket_dir):
    ...
    shutil.move((out),(os.path.join(pocket_dir,'pocket_conformations')))

def extract_centroid_probe(centroid_frame,cluster_number,cluster_pop,probe,pocket_name,pocket_dir,totframes):
#    subset = probe + '_poses.maegz'
    subset = 'all_ligands.maegz'
    clpop = float(cluster_pop)/float(totframes)
    #relative to -8kcal/mol
    clene = (-1*np.log(clpop)*(0.001987*300))-8.0
    #pactivity
    clactivity = (-1*np.log10(np.exp(clene/(0.001987*300))))
    st = structure.StructureReader.read(subset, index=centroid_frame)
    out = (pocket_name + '_' + probe + '_frm' + str(centroid_frame)+"_cluster_"+str(cluster_number)+"_pop_"+str(cluster_pop)+".maegz") 
    with structure.StructureWriter(out) as writer:
        ...
        if not sliced_done:
            ...
            for i in range(run_num):
                ...
                if i == 0:
                    ...
                    hotframetot=int(len(hotspot_frames_list) * run_num)
            ...
            if not os.path.isfile(str(solv)+'_poses.maegz'):
                ...
                if subsets_found:
                   ...
                   if not os.path.isfile('clustered_ligand1.out'):
                        conformer_cluster_cosolv(solv,pdb)
...
#                        print(cluster_pops[cent[1]],'clusterpops')
                        ...
                        extract_centroid_probe(int(cent[0]),int(cent[1]),int(cluster_pops[cent[1]]),solv,pname,pocket_dir, hotframetot)
